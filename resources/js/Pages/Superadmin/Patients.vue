                ...
                <v-form ref="form" v-model="valid">
                  <v-card class="py-5">
                    ...
                    <v-card-text class="px-5">
                      <v-text-field
                        label="Allergies"
                        v-model="patient_items.allergies"
...
export default {
  ...
  computed: {
          formTitle() {
            return this.editedIndex === -1 ? "New Patient Record" : "Edit Patient Record";
          },
        },
        watch: {
          dialog(val) {
        ...
        methods: {
          ...
          async PatientList() {
            ...
            if (res.status === 200) this.patients = res.data;
            else {
              this.$Notice.error({
                title: "Error",
          ...
          editItem(item) {
            ...
            this.patient_items = Object.assign({}, item);
            this.dialog = true;
          },
          deleteItem(item) {
            this.id = item.id;
          ...
          async deleteItemConfirm() {
            var res = await axios.delete("/patients/delete/" + this.id);
            if (res.status === 200) {
              this.PatientList();
              this.closeDelete();
            } else {
          ...
          close() {
            this.dialog = false;
            this.$nextTick(() => {
              this.patient_items = Object.assign({}, this.defaultItem);
              this.editedIndex = -1;
            });
          },
          closeDelete() {
          ...
          Submit() {
            if (this.$refs.form.validate()) {
              if (this.id) {
                axios
                  .put("/patients/update/" + this.id, this.patient_items)
                  .then((response) => {
                    this.PatientList();
                    this.close();
                  })
                ...
                axios
                  .post("/patients/create", this.patient_items)
                  .then((response) => {
                    this.PatientList();
                    this.close();
